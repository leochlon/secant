...
As far as we are aware, ours is the first framework realising the derivative-operator
framing of RoPE with an exact full-softmax effective-weight representation for arbitrary
projection weights. Previous attempts like RollPE only connect a special RoPE spectrum to
Fourier differentiation [6]; our combined framework additionally derives the full-softmax
effective matrix, exact for the deployed architecture. Our construction is the full-softmax
treatment overcoming the limitations of Von Oswald's linear-attention treatment of softmax
transformers [1]; and where related fast-weight [2] and implicit-gradient [3] derivations
linearize or remove softmax, we preserve full softmax, RoPE, and projection biases intact.
